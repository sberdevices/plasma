...
Текст – основа интерфейса. Типографическая система Plasma разработана, чтобы текст выглядел одинаково во всей экосистеме SberDevices. Типографическая система представляет собой набор констант ([_токенов_](./tokens)) и [компонентов](../components/typography). Компоненты заменяют обычные HTML-теги, такие как `h1`, `h2`, `h3`, `h4`, `p` и некоторые другие.
