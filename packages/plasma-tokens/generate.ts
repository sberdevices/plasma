...
const generateTokens = (tokens: TokenGroup<TokenType>, css?: boolean, cssPrefix?: string, type?: boolean) => {
    return Object.entries(tokens).reduce((acc, [key, token]) => {
        ...
        if (typeof value === 'string') {
            ...
            if (type) {
                const type = capit(key);
                // acc += `import type { Properties } from 'csstype'\n\n`;
                acc += `type ${type} = {\n${Object.keys(value)
                    .map((key) => `    ${key}: any`)
                    .join(`;\n`)};\n};\n\n`;
                acc += `export const ${key}: ${type} = ${objToStr};`;
            } else {
...
const generateTypography = (typography_dir: string, typo: Typo, tokens: boolean = false) => {
    ...
    for (let [name, styles] of Object.entries(text)) {
        ...
        const textTS = generateTokens({ [name]: { value: styles } }, true, `typo-${name}`, true);
