...
export const Slide = ({ product, onSlideHover, name, index, amount }: Props) => {
    ...
    return html`
        ...
        >
            ...
            <div
                class=${classNames('layer-swiper-slide-bottom', {
                    'layer-swiper-slide-bottom_stub': !product,
                    'layer-swiper-slide-bottom_secondary': template === TemplateEnum.LARGE,
                })}
            >
                ${product
                    ? html` <div class="layer-swiper-slide-bottom__shop-name">${product.retailer.name}</div>`
                    : ''}
                <div class="layer-swiper-slide-bottom__bg">
                    ${product
                        ? html`<div
                              class=${classNames('layer-swiper-slide-bottom__name', {
                    ...
                    ${product
                        ? html` <div class="layer-swiper-slide-bottom__price">${format(product?.price)} ₽</div>`
                        : html` <div class="layer__sketelon layer-w-50"></div>`}
                </div>
            </div>
        </div>
    `;
